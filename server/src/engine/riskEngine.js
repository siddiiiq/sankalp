...
function calculateRisk(input) {
  ...
  const ruleSets = [
    bpRules({ systolic: vitals.systolic, diastolic: vitals.diastolic }),
    sugarRules({ bloodSugar: vitals.bloodSugar }),
    bmiRules({ bmi: vitals.bmi }),
    ageRules({ age: input.age }),
    familyHistoryRules({ familyDiabetes: riskFactors.familyDiabetes, familyBP: riskFactors.familyBP }),
    lifestyleRules({ smoking: riskFactors.smoking, alcohol: riskFactors.alcohol, exercise: riskFactors.exercise, stress: riskFactors.stress }),
    symptomRules(symptoms),
    pregnancyRules({ isPregnant: pregnancy.isPregnant, gestationalHistory: pregnancy.gestationalHistory })
  ];
  ...
  totalScore = Math.min(Math.round(totalScore * 10) / 10, MAX_SCORE);
